...
subtest 'public_update() tests' => sub {
    ...
    my $update_data2 = {
        name                     => "Another Updated List Name",
        public                   => 0,
        allow_change_from_owner  => 1,
        allow_change_from_others => 0,
        default_sort_field       => 'title'
    };
    $t->put_ok( "//$patron_userid:$password@/api/v1/public/lists/" . $unmodifiable_list->id => json => $update_data2 )
        ->status_is( 200, "Owner can update list regardless of allow_change_from_owner flag" );
...
subtest 'public_delete() tests' => sub {
    ...
    $t->delete_ok( "//$patron_userid:$password@/api/v1/public/lists/" . $unmodifiable_list->id )
        ->status_is( 204, "Owner can delete list regardless of allow_change_from_owner flag" );

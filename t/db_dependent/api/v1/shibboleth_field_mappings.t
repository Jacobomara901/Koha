...
subtest 'list() tests' => sub {
    ...
    $t->get_ok(qq~//$userid:$password@/api/v1/shibboleth/mappings?q=[{"me.koha_field":{"like":"%user%"}}]~)
        ->status_is(200)
        ->json_is( [ $mapping_to_search->to_api ] );

    $t->get_ok("//$userid:$password@/api/v1/shibboleth/mappings?blah=blah")
        ->status_is(400)
...
subtest 'get() tests' => sub {
    ...
    $t->get_ok( "//$userid:$password@/api/v1/shibboleth/mappings/" . $mapping->mapping_id )
        ->status_is(200)
    ...
    $t->get_ok("//$userid:$password@/api/v1/shibboleth/mappings/$non_existent_id")
        ->status_is(404)
...
subtest 'add() tests' => sub {
    ...
    my $mapping_id =
        ...
        ->json_is( '/idp_field'  => $mapping->{idp_field} )
        ->json_is( '/koha_field' => $mapping->{koha_field} )
    ...
    $t->post_ok( "//$userid:$password@/api/v1/shibboleth/mappings" => json => $mapping_without_idp_or_default )
        ->status_is(400)
        ->json_is( "/error" => "Either idp_field or default_content must be provided" );
...
subtest 'delete() tests' => sub {
    ...
    $t->delete_ok("//$userid:$password@/api/v1/shibboleth/mappings/$mapping_id")

...
subtest 'print notice charging workflow with charging enabled' => sub {
    ...
    my $charge_result = $account->add_print_notice_charge({
    ...
    like($charge_result->description, qr/Print notice: ODUE/, 'Description includes notice code');
...
subtest 'multiple print notices for same patron' => sub {
    ...
    foreach my $code (@notice_codes) {
        ...
        my $charge = $account->add_print_notice_charge({
            notice_code => $code,
    ...
    # Verify each charge has correct notice code in description
    my @lines = $account->lines->as_list;
    my @descriptions = map { $_->description } @lines;
    like(join(' ', @descriptions), qr/ODUE/, 'ODUE charge description present');
    like(join(' ', @descriptions), qr/PREDUE/, 'PREDUE charge description present');
    like(join(' ', @descriptions), qr/HOLD/, 'HOLD charge description present');
...
subtest 'print notice charging with missing patron data' => sub {
    ...
    warnings_are {
        if ($found_patron) {
            ...
            $result = $account->add_print_notice_charge({
                notice_code => $message->{letter_code},
...
subtest 'print notice charging with different amounts' => sub {
    ...
    my $charge1 = $account->add_print_notice_charge({
        notice_code => 'ODUE',
        library_id => $patron->branchcode,
    });
    is($charge1->amount, 0.75, 'System preference amount used when no custom amount');

    # Test 2: Use custom amount
    my $charge2 = $account->add_print_notice_charge({
        notice_code => 'PREDUE',
        library_id => $patron->branchcode,
        amount => 1.50,
    });
    ...
    my $charge3 = $account->add_print_notice_charge({
        notice_code => 'HOLD',
        library_id => $patron->branchcode,
        amount => 0.00,
    });
    ...
    my $charge4 = $account->add_print_notice_charge({
        notice_code => 'RENEWAL',
        library_id => $patron->branchcode,
        amount => 0.01,
    });
...
subtest 'gather_print_notices.pl helper function tests' => sub {
    ...
    # Enable print notice charging
    t::lib::Mocks::mock_preference('PrintNoticeCharging', 1);
    t::lib::Mocks::mock_preference('PrintNoticeChargeAmount', '0.60');
    ...
    my @descriptions = map { $_->description } @lines;
    my $descriptions_text = join(' ', @descriptions);
    like($descriptions_text, qr/ODUE.*PREDUE|PREDUE.*ODUE/, 'Both notice codes in descriptions');
